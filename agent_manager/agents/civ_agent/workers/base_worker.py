...
class BaseWorker(ABC):
    ...
    def choose_action(self,
        ...
        while exec_action_name is None:
            if cur_times < max_try_times:
                try:
                    response = self.generate_command(input_prompt +
                                                     prompt_addition)
                    if self.llm.model_name.__contains__("llama3.1:70b") and avail_action_list!=['suggestion']:
                        # try:
                        #     self.parse_response(response)
                        # except :
                        post_prompt = """
                        Input:\n  {{response}}  
                        Output: change format of the string to the following:
                        ```json
                        {
                            "thoughts": {
                        ...
                        post_prompt = self.format_prompt(post_prompt, {"response": response})
                        post_messages = [{"role": "user", "content": post_prompt}]
                        response = self.llm.query(post_messages)
                        print(response)
                    exec_action_name, prompt_addition = self.process_command(
                        response, input_prompt, avail_action_list)

                    self.dialogue += [{'role':'assistant','content':response}]
