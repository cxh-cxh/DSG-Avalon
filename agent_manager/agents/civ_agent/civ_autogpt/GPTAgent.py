    ...
    def query(self, stop=None, temperature=0.7, top_p=0.95):
        ...
        if self.model in ['gpt-3.5-turbo-0301', 'gpt-3.5-turbo']:
            ...
            chat = ChatOpenAI(model_name='deepseek-chat',
                              openai_api_key='sk-c7743bb83dc54d078af3731f4d7885e1',
                              temperature=0.2,
                              max_tokens=1024,
                              n=1,
                              request_timeout=60,
            ...
            generations = chat.generate([self.dialogue], stop=[stop] if stop is not None else None)
            response = [chat_gen.message.content for chat_gen in generations.generations[0]][0]
