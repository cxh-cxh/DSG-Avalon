...
import yaml
from box import Box
from games.welfare_diplomacy.diplomacy import Game
import wandb

from games.welfare_diplomacy.welfare_diplomacy_baselines.environment import mila_actions, diplomacy_state
from games.welfare_diplomacy.welfare_diplomacy_baselines.baselines import no_press_policies
...
class LLMAgent2(Agent):
    ...
    def __init__(self, model_name: str, **kwargs):
        ...
        args={
            ...
            "api_key":"sk-e851421cc0224deb8fad0c8a30d5c862",
            "api_url":"https://api.deepseek.com",
            "max_tokens":4096,
            "timeout":60,
            "temperature":0.2
        }
        from agent_manager.llm_models.llm_model import LLMModel
        from openai import OpenAI
        llm_model=LLMModel(args)
        self.backend=OpenAI(api_key=llm_model.api_key,base_url=llm_model.api_url)
...
class LLMAgent(Agent):
    ...
    def __init__(self, model,logger=None, role=None,**kwargs):
        self.logger=logger
    ...
    def __repr__(self) -> str:
        return f"LLMAgent(Backend: {self.llm_model.model_name}, Temperature: {self.llm_model.temperature})"
    ...
    def respond(self, params: AgentParams) -> AgentResponse:
        ...
        try:
            ...
            orders = completion["orders"]
            self.update_traj_action(str(orders))
            # Clean orders
            for order in orders:
                if not isinstance(order, str):
                    raise AgentCompletionError(
    ...
    def update_traj_action(self,content):
        if len(self.backend.trajectory)>=1:
            self.backend.trajectory[-1]['content']=content
class LLMAgentNew(Agent):
    """Uses OpenAI/Claude Chat/Completion to generate orders and messages."""
    ...
    def __init__(self, model,logger=None,role=None, **kwargs):
        ...
        self.use_completion_preface = False
        self.llm_mode=model
        self.backend = OpenAIChatBackend(model=model,logger=logger,role=role)

    def __repr__(self) -> str:
        return f"LLMAgent(Backend: {self.llm_mode.model_name}, Temperature: {self.llm_mode.temperature})"
...
class ExploiterAgentNew(Agent):
    ...
    def __init__(self, llm_model,logger=None,role=None, **kwargs):
        self.logger=logger
        """Instantiate LLMAgent and RL policies."""
        self.center_threshold = kwargs.pop("center_threshold")
        self.unit_threshold = kwargs.pop("unit_threshold")
        ...
        self.rl_policy = no_press_policies.get_network_policy_instance()
        # Set intial "cooperative" policy
        self.llm_policy = LLMAgentNew(llm_model,logger,role, **kwargs)
        self.exploiting = False
        self.triggered = False
        self.start_disbanding = False
