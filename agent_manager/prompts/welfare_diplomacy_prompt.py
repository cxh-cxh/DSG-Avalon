...
from games.welfare_diplomacy.diplomacy import Power
...
# You will get the message history that this player saw for the most recent phase which is {params.game.phase} ({params.game.get_current_phase()}). Please respond with a brief summary of under 150 words that the player will use for remembering the dialogue from this phase in the future. Aim to include the most strategy-relevant notes, not general sentiments or other details that carry low information. Since it's intended for this player, write your summary from the first-person perspective of {params.power.name.title()}. Respond with just the summary without quotes or any other text."""
    ret=rf"""You will be helping out an expert AI playing the game Diplomacy as the power {params.power.name.title()}.{welfare_rules}
    ...
    please based on the dialogue information and turn orders, summarize the dialogue from this phase and extract the social relations of the power Austria, the set of countries that initiated negotiations, and the set of countries that agreed to the alliance,\
    \n\n Here are examples showing the format :{{"summary":"a brief summary of under 150 words that the player will use for remembering the dialogue from this phase in the future",\\
    "negotiations_powers":["List of strings of powers that initiated negotiations，without the {params.power.name.title()}"],\
    "allied_powers":["List of strings of powers that agreed to the alliance，without the {params.power.name.title()}"],\
    "betrayal_powers":["List of strings of powers that considered as betraying powers，without the {params.power.name.title()}"]}}
    """
    return ret
