...
class Zerg_Bot(BotAI):
    ...
    def get_enemy_unity(self):
        if self.enemy_units:
            ...
            for unit in self.enemy_units:
                unit_type = 'enemy_' + str(unit.type_id)  # Add prefix 'enemy_' to the key
    ...
    def get_enemy_structure(self):
        if self.enemy_structures:
            ...
            for structure in self.enemy_structures:
                structure_type = 'enemy_' + str(structure.type_id)  # Add prefix 'enemy_' to the key
    ...
    def get_information(self):
        self.worker_supply = self.workers.amount  # Number of workers
        self.army_supply = self.supply_army  # Army population
        self.base_count = self.structures(UnitTypeId.HATCHERY).amount + self.structures(
            UnitTypeId.LAIR).amount + self.structures(UnitTypeId.HIVE).amount  # Number of bases
        self.base_pending = self.already_pending(UnitTypeId.HATCHERY)  # Number of bases under construction
        self.overlord_count = self.units(UnitTypeId.OVERLORD).amount  # Current number of Overlords
        self.overlord_planning_count = self.already_pending(UnitTypeId.OVERLORD)  # Number of Overlords under construction

        self.queen_count = self.units(UnitTypeId.QUEEN).amount  # Current number of Queens
        self.Evolution_chamber_count = self.structures(UnitTypeId.EVOLUTIONCHAMBER).amount  # Number of Evolution Chambers
        self.Spire_count = self.structures(UnitTypeId.SPIRE).amount  # Number of Spires
        self.Roach_warren_count = self.structures(UnitTypeId.ROACHWARREN).amount  # Number of Roach Warrens
        self.Baneling_nest_count = self.structures(UnitTypeId.BANELINGNEST).amount  # Number of Baneling Nests
        self.Nydus_Network_count = self.structures(UnitTypeId.NYDUSNETWORK).amount  # Number of Nydus Networks
        self.Nydus_Worm_count = self.structures(UnitTypeId.NYDUSWORMLAVADEATH).amount  # Number of Nydus Worms
        self.Greater_spire_count = self.structures(UnitTypeId.GREATERSPIRE).amount  # Number of Greater Spires
        self.Hydralisk_den_count = self.structures(UnitTypeId.HYDRALISKDEN).amount  # Number of Hydralisk Dens
        self.Lurker_Den_count = self.structures(UnitTypeId.LURKERDENMP).amount  # Number of Lurker Dens
        self.Lair_count = self.structures(UnitTypeId.LAIR).amount  # Number of Lairs
        self.Hive_count = self.structures(UnitTypeId.HIVE).amount  # Number of Hives
        self.Ultralisk_Cavern_count = self.structures(UnitTypeId.ULTRALISKCAVERN).amount  # Number of Ultralisk Caverns
        self.gas_buildings_count = self.gas_buildings.amount  # Number of gas buildings
        self.gas_buildings_planning_count = self.already_pending(UnitTypeId.EXTRACTOR)  # Number of gas buildings under construction
        self.overseer_count = self.units(UnitTypeId.OVERSEER).amount  # Current number of Overseers
        ...
        information = {
            ...
            "research":{
                # Research status: 1 indicates completed, 0 indicates not started, values between 0-1 indicate research progress
                'zergling_movement_speed_research_status': self.already_pending_upgrade(
                    UpgradeId.ZERGLINGMOVEMENTSPEED),
                'zergling_attack_speed_research_status': self.already_pending_upgrade(UpgradeId.ZERGLINGATTACKSPEED),
                'zerg_ground_armor_level_1_research_status': self.already_pending_upgrade(
                    UpgradeId.ZERGGROUNDARMORSLEVEL1),
                'zerg_ground_armor_level_2_research_status': self.already_pending_upgrade(
                    UpgradeId.ZERGGROUNDARMORSLEVEL2),
                'zerg_ground_armor_level_3_research_status': self.already_pending_upgrade(
                ...
                'evolve_muscular_augments_research_status': self.already_pending_upgrade(
                ...
                'zerg_flyer_armor_level_1_research_status': self.already_pending_upgrade(
                    UpgradeId.ZERGFLYERARMORSLEVEL1),
                'zerg_flyer_armor_level_2_research_status': self.already_pending_upgrade(
                    UpgradeId.ZERGFLYERARMORSLEVEL2),
                'zerg_flyer_armor_level_3_research_status': self.already_pending_upgrade(
    ...
    def get_process_data(self):
        ...
        process_data={
            ...
            "completed_tech":sum(1 for count in self.transaction['information']['building'].values() if count > 0)+sum(1 for researches in self.transaction['information']['research'].values()  if researches == 1)
        }
        print(process_data)
        return process_data
